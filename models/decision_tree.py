...
def train_and_evaluate_model(X_train, X_test, y_train, y_test, params=None):
    """Trains a Decision Tree model, evaluates it, and prints the results.
...
def visualize_results(y_test, y_pred_prob, model, feature_names):
  """Visualizes the ROC curve and decision tree (if the tree's depth is not too high).
  ...
  # Visualize Decision Tree if the depth is not too high
  if model.get_depth() <= 5:  # Limit the depth to avoid large, illegible trees
...
def main():
    ...
    # Visualize Results
    visualize_results(y_test, y_pred_prob, model, feature_names)
